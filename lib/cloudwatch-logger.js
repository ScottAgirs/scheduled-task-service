...
async function logToCloudWatch(message, level = "INFO", additionalData = {}, streamName = "default") {
  try {
...
async function initializeCloudWatchLogs(streamName = "default") {
  try {
    ...
    console.log(`✅ CloudWatch logging initialized for ${LOG_GROUP_NAME}/${streamName} and ${ERROR_LOG_GROUP_NAME}/${streamName}`);
  } catch (error) {
