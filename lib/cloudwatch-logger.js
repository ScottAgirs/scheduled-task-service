...
async function initializeCloudWatchLogs(streamName, interval = "unknown", timestamp = "unknown") {
  try {
    ...
    for (const logGroupName of logGroups) {
      ...
      try {
        await cloudWatchLogs.send(new CreateLogStreamCommand({
          logGroupName,
          logStreamName: streamName || "default"
        }));
      } catch (error) {
        if (error.name !== "ResourceAlreadyExistsException") {
          throw error;
    ...
    console.log(`✅ CW initialized. Logs: ${logGroupName}/${streamName ? streamName : `Default: "default"`}, Interval: ${interval}, Timestamp: ${timestamp}`);
  } catch (error) {
